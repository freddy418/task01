...
namespace HomeworkTask01
{
    ...
    public class Task01_Naive1
    {
	    ...
	    int year = int.Parse(tag779.Substring(0, 4));
            ...
            string LastUpdateTime = (dt - epochDate).TotalSeconds + tag779.Substring(14, 6);
		...
		if (tag1148[tag1148.Length - 8] != '.') throw new Exception("Bug");
            	...
            	string lp = tag1148.Replace(".", "");
            	string hp = tag1149.Replace(".", "");
            	long lLowPrice = long.Parse(lp);
            	long lHighPrice = long.Parse(hp);
	    ...
	    string[] sbs = new string[numlines];

            Parallel.For (0, numlines, new ParallelOptions {MaxDegreeOfParallelism = 8}, index =>
            {
                sbs[index] = ParseLine(lines[index]);
            });
	    File.WriteAllLines(outputPath, sbs);
        }
